...
const MxPlayerPage= () => {
     const [movies, setMxPlayerContent] = useState([]);
    ...
    useEffect(() => {
        const fetchMxPlayerContent = async () => {
            ...
            try {
                ...
                // Filter for MxPlayer and reverse to show latest first
                const mxPlayerOnly = data
                    .filter(movie => movie.platform === 'MxPlayer')
                    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)); // Sort by latest
                setMxPlayerContent(mxPlayerOnly);

            } catch (error) {
                console.error('Error fetching MxPlayer content:', error);
